...
import { getProduct, updateProduct, getCategories, uploadProductImage } from '@/lib/actions/products'
...
export default function ProductEditPage() {
  ...
  const {
    ...
    defaultValues: {
      ...
      category_id: '', brand_id: '', cost_price: 0, sale_price: 0,
  ...
  React.useEffect(() => {
    ...
    getProduct(productId).then((p) => {
      ...
      reset({
        ...
        unit: (p.unit as ProductFormValues['unit']) ?? 'un',
  ...
  // Update inheritedNcm when category changes
  React.useEffect(() => {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="border-b border-border bg-background px-6 py-4">
        ...
        <h1 className="mt-3 text-2xl font-bold tracking-tight">Editar produto</h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col flex-1">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    router.push(`/kyra?q=${encodeURIComponent(q)}` as never)
                  }}
          ...
          {activeTab === 'extra' && (
            <div className="flex flex-col gap-4 max-w-2xl">
              <div className="flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3">
                <div>
                  <Label htmlFor="edit_is_active" className="text-sm font-medium">Produto ativo</Label>
                  <p className="text-xs text-muted-foreground mt-0.5">Visível e disponível para venda</p>
                </div>
                <Switch id="edit_is_active" checked={isActive} onCheckedChange={(v) => setValue('is_active', v)} />
              </div>
              <div className="flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3">
                <div>
                  <Label htmlFor="edit_is_featured" className="text-sm font-medium">Produto em destaque</Label>
                  <p className="text-xs text-muted-foreground mt-0.5">Exibir em seções de destaque</p>
                </div>
                <Switch id="edit_is_featured" checked={isFeatured} onCheckedChange={(v) => setValue('is_featured', v)} />
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 border-t border-border bg-background/95 backdrop-blur px-6 py-4 flex items-center justify-end gap-3 z-20">
          <Button type="button" variant="outline" onClick={() => router.push('/products')} disabled={submitting}>
            Cancelar
          </Button>
